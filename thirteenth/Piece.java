...
class Piece {
    ...
    Direction findNextDirection(int myID, GeneralGameInformation gameInformation) {
        ...
        int prodGoal = (int) gameInformation.getProductionStatistics().getPercentile(80);
...
//        Site nextSite = gameMap.getSite(this.getLoc(), nextNpcDirection);

        Piece enemyNeighbor = getEnemyNeighbor(myID);

        if (!enemyNeighbor.isNil()){
            retDirection = gameMap.getFromPieceToPiece(this, enemyNeighbor);
...
//    }
    ...
    private Piece getEnemyNeighbor(int myID) {
...
//                hasCornerwiseEnemyNeighbor(myID, Direction.NORTH);
        for (Direction direction : Direction.CARDINALS) {
            if (hasCornerwiseEnemyNeighbor(myID, direction))
                piece = Piece.fromLocation(gameMap.getLocation(getLoc(), direction), gameMap);
        }
        return piece;
    }
